...
export class CreateUsersTable1729130798518 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`users\` ADD CONSTRAINT \`FK_7ae37bc9c3aa8f0d7d4e1c2b71f\` FOREIGN KEY (\`partnerId\`) REFERENCES \`users\`(\`id\`) ON DELETE SET NULL ON UPDATE NO ACTION`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`users\` DROP FOREIGN KEY \`FK_7ae37bc9c3aa8f0d7d4e1c2b71f\``,
    );
    await queryRunner.query(
      `DROP INDEX \`REL_7ae37bc9c3aa8f0d7d4e1c2b71\` ON \`users\``,
    );
    await queryRunner.query(
      `DROP INDEX \`IDX_6f3b146c7345da5f7ec474edd4\` ON \`users\``,
    );
    await queryRunner.query(
      `DROP INDEX \`IDX_da8b40ba8bc48f3be743d48673\` ON \`users\``,
    );
    await queryRunner.query(
      `DROP INDEX \`IDX_4f957cab9b20675e7a50a7ebfc\` ON \`users\``,
    );
