...
export class ChangeKeyNameOfTracker1730215420624 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`mom_infos\` CHANGE \`thumbnail3D\` \`thumbnail3DUrl\` varchar(255) NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`baby_infos\` CHANGE \`thumbnail3D\` \`thumbnail3DUrl\` varchar(255) NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`baby_infos\` CHANGE \`height\` \`high\` double NOT NULL`,
    );
  }
